...
class fileCache {
		...
		private function APP_start ($app_file='application')
		{
		    ...
		    { // if data file exists, load the cached variables
			    ...
			    {//not expired
	                ...
	                if ($filesize>0)
	                {
	                    $data = H::fread($this->cache_path.$this->application_file);
	                    if ($data)
	                    {
	        ...
	        {//if the file does not exist we create it
	            ...
	            H::fwrite($this->cache_path.$this->application_file, '');
	        }
    ...
    {
        H::remove_resource($_target,$older_than);
    }
}
