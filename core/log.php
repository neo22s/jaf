...
class log{
    ...
    {
        $db=DB::get_instance();
        $ret='Page generated the '.date('d M Y H:i:s').' in '.self::show_timer(3).'s. Total queries: '.$db->get_counter().'.';
        if (DEBUG)//all the info
        {
            $ret.=' Total cached queries: '.$db->get_counter('cache').'. Memory usage: '.self::show_memory('mb',5).'mb.';
        }
        self::stop_timer();
        if (DEBUG)
        {
	...
	public static function error_handler($errno, $errstr, $errfile, $errline)
    {
        ...
        switch ($errno) {
            case E_USER_ERROR:
            	...
            	{
            	    $error_type = 'SQL';
                    $error_details = DB::get_error().' '.$error_details;
                } 
                else 
                {
                    $error_type = 'CRITICAL';
                ...
                $msg = 'JAF: '. $error_type.$error_details;
        		error_log($msg, 0);
    ...
    /**
     * NOTE, this class has been refactored to be part of the JAF core.
     * 
